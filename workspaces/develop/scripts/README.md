...
5. **Container registry**: Prompts for container registry login
...
7. **Builds components**: Builds and deploys kubeflow notebooks components with custom images:
   - controller: `${registry}/kubeflow-notebooks-v2:controller-${tag_suffix}` (via make)
   - backend: `${registry}/kubeflow-notebooks-v2:backend-${tag_suffix}` (via make)
   - frontend: `${registry}/kubeflow-notebooks-v2:frontend-${tag_suffix}` (via npm + docker + kustomize)
...
4. **Check network**: Verify internet connectivity for downloading components
...
- Frontend uses manual build process (npm + docker + kustomize) as it doesn't have a Makefile
