...
login_container_registry() {
    ...
    indent_output $CONTAINER_ENGINE login docker.io

    log_success "Container registry login completed"
}
...
install_poddefaults() {
    ...
    # Wait for PodDefaults to be ready
...
build_and_deploy_controller() {
    ...
    if [ "$force_mode" = true ] || [ "$image_exists" = false ]; then
        ...
        indent_output IMG="$img" CONTAINER_TOOL="$CONTAINER_ENGINE" $build_args $MAKE_CMD docker-build
    else
...
build_and_deploy_frontend() {
    ...
    if [ "$force_mode" = true ] || [ "$image_exists" = false ]; then
        ...
        indent_output $CONTAINER_ENGINE build $build_args -f Dockerfile -t "$img" .
    else
