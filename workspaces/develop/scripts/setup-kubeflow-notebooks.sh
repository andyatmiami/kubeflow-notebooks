...
build_and_deploy_controller() {
    ...
    if [ "$force_mode" = true ] || [ "$image_exists" = false ]; then
        ...
        local build_args=""
        if [ "$force_mode" = true ]; then
            build_args="CONTAINER_BUILD_ARGS=--no-cache"
        fi

        # Use retry mechanism for docker build
        retry_with_backoff "IMG=\"$img\" $build_args $MAKE_CMD docker-build"
    else
    ...
    indent_output kubectl rollout restart deployment workspaces-controller -n kubeflow-workspaces

    # Validate controller deployment
    log_info "Validating controller deployment..."
    indent_output kubectl wait --for=condition=ready pod -l app.kubernetes.io/component=controller-manager -n kubeflow-workspaces --timeout=300s
